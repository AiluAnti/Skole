...
def test_throughput_put(nodes):
    ...
    t0 = time.time()
    for key, value in pairs.iteritems():
        if round(time.time() - t0, 1) >= testtime:
            print "Did %d PUT and GET requests in %d seconds with %d nodes" % (successes, testtime, len(nodes))
            f = open('workfile', 'a')
            f.write("\nNumber of nodes:")
            f.write(str(len(nodes)))
            f.write("\nNumber of PUT and GET requests:")
