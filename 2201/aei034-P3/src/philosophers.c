...
/*typedef struct {
lock_t *lock;
condition_t *philosophers[3];
int status[3]; // Taken / Free
} monitor_t;

void monitor_init(monitor_t * m);

void monitor_init(monitor_t * m, int philosophers)
{
    m->lock->status = UNLOCKED;
 ...
 */
static void
update_keyboard_LED (unsigned char bitmask)
{
...
void num(void)
{
    ...
    while (1) 
    {
        ...
        n_eat++;
        print_str(PHIL_LINE, PHIL_COL + 60, "NUM: ");
        print_int(PHIL_LINE, PHIL_COL + 70, n_eat);
...
void caps(void)
{
    ...
    while (1) 
    {
        ...
        eat_for_a_random_time ();
        c_eat++;
        print_str(PHIL_LINE - 1, PHIL_COL + 60, "CAPS: ");
        print_int(PHIL_LINE - 1, PHIL_COL + 70, c_eat);
...
void scroll_th(void)
{
    ...
    while (1) 
    {
        ...
        eat_for_a_random_time ();
        s_eat++;
        print_str(PHIL_LINE - 2, PHIL_COL + 60, "SCROLL: ");
        print_int(PHIL_LINE - 2, PHIL_COL + 70, s_eat);
