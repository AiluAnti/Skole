...
#define ARGS "[--extended] [--vm]" \
" [--fs] [--kernel] <bootblock> <executable-file> ..."
...
static void reserve_fs_blocks(struct image_t *im, int fs_blocks);
...
static void create_images(int nfiles, char *files[])
{
	...
	/*
	 * if with vm, the os size will be only the size of
	 * kernel, if any
	 */
	write_os_size(&image);
    ...
    if (options.fs == 1) {
	/* reserve some blocks for the filesystem. */
	reserve_fs_blocks(&image, 512 + 2);
    }
    ...
    if (options.vm == 0) {
	/*
	 * if there is no vm, the os size will include all the
	 * processes
	 */
	write_os_size(&image);
    }
...
static void process_start(struct image_t *im, int vaddr)
{
     ...
     * should be aligned to sector boundary before calling it, so
     * let's check it
     */
			  ...
			  Elf32_Phdr phdr)
{
     ...
     * padding here is removed to process_end, this will allow two
     * segments to reside in the same sector
     */
}
...
static void write_process_directory(struct image_t *im)
{
	...
	error("Too many processes! Can't hold them "
	      "in the directory!\n");
    }
...
static void reserve_fs_blocks(struct image_t *im, int fs_blocks)
{
    ...
    fseek(im->img, 0, SEEK_END);
    left = fs_blocks * SECTOR_SIZE + 1;
    while (--left)
	if (fputc(0, im->img) == EOF)
	    break;
    if (left)
	error("Unable to reserve %d blocks for filesystem\n",
	      fs_blocks, left);
    im->nbytes += fs_blocks * SECTOR_SIZE;
    /* TODO: print which blocks were reserved */
    if (options.extended == 1)
