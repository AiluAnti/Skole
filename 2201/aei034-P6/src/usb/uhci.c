...
td_build(struct usb_pipe *pipe, uint8_t pid, int size, char *data) {
  ...
  td->control_status = 
    (usb_dev_speed << UHCI_TD_LS_DEV_OFF) |  /* USB device speed                          */
    (3 << UHCI_TD_ERROR_COUNTDOWN_OFF) |     /* Allow up to three errors in transmission  */
    (interrupt_pipe  << UHCI_TD_IOC_OFF) |   /* Generate Interrupt on transfer complete
 ...
 *    this UHCI schedule 
 */
static void xfer_enqueue(struct uhci_transfer_unit *xfer,
                  struct uhci *uh,
  ...
  if ((uhci_qh->vert_lp & UHCI_LP_TERMINATE) != 0) 
     ...
     */
    next_qh = (struct uhci_queue_head *)(uhci_qh->vert_lp & UHCI_LP_ADDR_MASK);
...
static int uhci_read_write(struct usb_pipe *pipe, int pid, int size, char *data) {
  ...
  do {
    packet_size = remaining_size > pipe->max_packet_size ?
      pipe->max_packet_size : remaining_size;
   ...
   */
  switch (pid) {
    case UHCI_TD_PID_IN: xfer_enqueue(xfer_container, uh, uh->bulk_in_qh); break;
    case UHCI_TD_PID_OUT: xfer_enqueue(xfer_container, uh, uh->bulk_out_qh); break;
    case UHCI_TD_PID_SETUP: xfer_enqueue(xfer_container, uh, uh->control_qh); break;
...
enum port_status_e uhci_port_status(struct usb_hub *uhub, int port) {
  ...
  if ((status & UHCI_PORTSC_CURRENT_CONNECT) == 0)
    ...
    if ((status & UHCI_PORTSC_ENABLE) == 0)
      rc = USB_PORT_DISABLED;
    else
      if ((status & UHCI_PORTSC_SUSPEND) == 0)
          rc = USB_PORT_ENABLED;
        else
  ...
  switch(command) {
    ...
    case USB_PORT_ENABLE:
    {
      ...
      do {
        ...
        ms_delay(1);
      } while (((value & UHCI_PORTSC_ENABLE) == 0) && (value & UHCI_PORTSC_CURRENT_CONNECT));
...
enum usb_speed_class_e uhci_port_speed(struct usb_hub *uhub, int port) {
  ...
  return  ((uhci_pci_read_portsc(uh->iobase, port) &
    UHCI_PORTSC_LS_DEV) != 0) ? USB_LS_DEV : USB_FS_DEV;
}
...
//  uh->bulk_out_qh->horiz_lp = ((uint32_t)uh->control_qh) | UHCI_LP_QH;
  ...
  /* Enable UHCI */
  uhci_pci_write(uh->iobase, UHCI_COMMAND, 0x00C1); /* 0x80 64B packets allowed at SOF 
