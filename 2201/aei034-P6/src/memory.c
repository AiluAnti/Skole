 ...
 * application.
 */
inline void table_map_page(uint32_t * table, uint32_t vaddr,
 ...
 * address 'paddr'. 'mode' sets bit [12..0] in the page table entry.
 */
inline void dir_ins_table(uint32_t * directory, uint32_t vaddr,
			  void *table, uint32_t mode)
{
...
void init_memory(void)
{
    ...
    page_set_mode(kernel_pdir, (uint32_t) SCREEN_ADDR,
		  PE_P | PE_RW | PE_US);
}
...
void setup_page_table(pcb_t * p)
{
	...
	for (i = 0; i < N_KERNEL_PTS; i++) {
	    dir_ins_table(pde, PTABLE_SPAN * i, kernel_pts[i],
			  PE_P | PE_RW | PE_US);
	}

	/* map process page table into process page directory */
	dir_ins_table(pde, PROCESS_START, page_addr(ptbl),
		      PE_P | PE_RW | PE_US);

	/* map stack page table into process page directory */
	dir_ins_table(pde, PROCESS_STACK, page_addr(stkt),
		      PE_P | PE_RW | PE_US);
	...
	for (i = 0; i < n_img_pages; i++) {
	    ...
	    table_map_page(pte, PROCESS_START + i * PAGE_SIZE,
			   PROCESS_START + i * PAGE_SIZE, PE_RW | PE_US);
	}

	pte = page_addr(stkt);
	/* map two stack pages into stack page table */
	table_map_page(pte, PROCESS_STACK, (uint32_t) page_addr(stkp1),
		       PE_P | PE_RW | PE_US);
	table_map_page(pte, PROCESS_STACK - PAGE_SIZE,
		       (uint32_t) page_addr(stkp2), PE_P | PE_RW | PE_US);
    }
...
void page_protection_error(uint32_t pde, uint32_t pte)
{
    ...
    i++;
    PRINT_INFO(i, "Hardware mask %04x",
...
static void page_swap_in(int pageno)
{
    ...
    int sector = page_disk_sector(page), nsectors;

    scrprintf(23, 50, "pid %-3d rding page %-3d",
	      current_running->pid, pageno);
...
static void page_swap_out(int pageno)
{
    ...
    scrprintf(24, 50, "pid %-3d wting page %-3d",
	      current_running->pid, pageno);
