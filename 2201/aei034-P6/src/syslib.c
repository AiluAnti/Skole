...
{
    return invoke_syscall(SYSCALL_FS_LINK, (int) linkname,
			  (int) filename, IGNORE);
}

int fs_unlink(char *linkname)
{
    return invoke_syscall(SYSCALL_FS_UNLINK, (int) linkname, IGNORE,
			  IGNORE);
}
...
{
    return invoke_syscall(SYSCALL_FS_MKDIR, (int) dir_name, IGNORE,
			  IGNORE);
}
