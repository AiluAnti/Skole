...
static void print_trace(char *s, int q, int msgSize)
{
    ...
    static int count = 0;
    count++;
    scrprintf(count % 10, 40, "%-5d%-5d%-10s%-10d%-10d",
	      count / 10, q, s, space_available(&Q[q]), msgSize);
#endif				/* DEBUG */
}
...
void print_mbox_status(void)
{
    ...
    scrprintf(LINE - 1, COL, "%-8s%-8s%-8s%-8s%-8s",
	      "Mbox", "Used", "Head", "Tail", "Count");

    for (i = 0; i < MAX_MBOX; i++)
	scrprintf(LINE - 1, COL, "%-8d%-8d%-8d%-8d%-8d",
		  i, Q[i].used, Q[i].head, Q[i].tail, Q[i].count);
}
