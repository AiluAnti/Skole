...
/*
* This function initializes the keyboard by using mailboxes
*
*/
...
int getchar(int *c)
{
  ...
  // Fetch mailbox from queue 'QUEUE' and store it in m
  mbox_recv(QUEUE, &m);

  // Letter to read will be the first and only in the message body
  letter = m.body[0];
  
  // Need to typecast because *c is an int
  *c = (int)letter;
...
void putchar(struct character *c)
{
  ...
  // Creating a char array with a little more space than needed
  char data[8];
  
  // Create a message of size 1(sizeof char)
  msg_t *m = data;
  m->size = 1;
  // This will be the only character in the message
  m->body[0] = c->character;

  mbox_stat(QUEUE, &count, &space);
  
  // We only send the message if there is space for it at the receiving buffer
  if (space >= MSG_SIZE(m))
    mbox_send(QUEUE, m);
