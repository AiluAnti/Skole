...
/*
 * Called by the kernel to initialize keyboard handling. Note that it
 * uses the mailboxes, so this must be initialized after the
 * mailboxes.
 */
...
/*
 * Function to read a character from the keyboard queue.  Writes an
 * ASCII character to *c.
 */
int getchar(int *c);
