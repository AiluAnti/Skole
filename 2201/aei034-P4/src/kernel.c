...
static void init_idt(void)
{
  ...
  create_gate(&(idt[IRQ_START + 7]),
        (uint32_t) fake_irq7_entry, KERNEL_CS, INTERRUPT_GATE,
        0);
  ...
  create_gate(&(idt[IDT_SYSCALL_POS]),
        (uint32_t) syscall_entry, KERNEL_CS, INTERRUPT_GATE,
        3);
...
int readdir(unsigned char *buf)
{
  ...
  short size;
  char *new_size = (char *)&size;
  
  // Read the bootblock sector
  scsi_read(0, 1, sector);

  // Copy size of kernel
  bcopy(&sector[2], new_size, 2);

  // Read the program directory (one sector)
  scsi_read((short)size + 1, 1, sector);
  
  // Copy the program directory into the buf
  bcopy(sector, (char *)buf, SECTOR_SIZE);
  return 1;
}
...
{
  // Allocate memory enough for a process
  uint32_t mem = alloc_memory((size * SECTOR_SIZE) + STACK_SIZE);

  // Read the process into memory
  scsi_read(location, size, (char *)mem);

  // Create the process
  create_process(mem,( size * SECTOR_SIZE) + STACK_SIZE);
