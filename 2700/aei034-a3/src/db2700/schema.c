...
static int int_x_larger (int x, int y )
{
    return x > y;
}

static int int_y_larger (int x, int y )
{
    return x < y;
}

static int int_x_larger_or_equal (int x, int y)
{
    return x >= y;
}

static int int_y_larger_or_equal (int x, int y)
{
    return y >= x;
}
...
int binary_find_record_int_val (record r, schema_p s, int offset, int (*op) (int, int), int val) {
    int pos, rec_val, first_blk, last_blk, curr_blk, old_rec_val;
    page_p first = get_page_for_next_record(s);
    page_p last = get_page(s->name, -1);
    first_blk = page_block_nr(first);
    last_blk = page_block_nr(last);
    page_p curr = get_page(s->name, (last_blk + first_blk) / 2);
    curr_blk = page_block_nr(curr);

    page_set_pos_begin(curr);
    pos = page_current_pos(curr);

    rec_val = page_get_int_at(curr, pos + offset);
    old_rec_val = rec_val;

    if (first == NULL) return 0;
    while (1) {
        // First int at current page
        rec_val = page_get_int_at(curr, pos + offset);

        // If the first value is what we're searching for
        if ( (*op) (val, rec_val) ) {
            page_set_current_pos(curr, pos);
            get_page_record(curr, r, s);
            return 1;
        }
        // If there is only one page
        else if (get_page_num(first) == get_page_num(last)) {
            break;
        } else if (val > rec_val) {
            if (val < old_rec_val) {
                search_pages(curr, val, s, r, offset);
                return 1;
            }
            // If the value we look for is bigger than the one at hand, we need to go back pages
            old_rec_val = rec_val;
            first_blk = page_block_nr(curr);
            curr = get_page(s->name, ((last_blk + curr_blk) / 2) + 1);
            curr_blk = first_blk;

            if (last_blk - curr_blk == 0) {
                search_pages(curr, val, s, r, offset);
                return 1;
            }

        } else if (val < rec_val){
            // If the value we look for is smaller than the one at hand, we need to move forward
            old_rec_val = rec_val;
            last_blk = page_block_nr(curr);
            curr = get_page(s->name, ((first_blk + curr_blk) / 2));
            curr_blk = last_blk;
...
int search_pages(page_p curr, int val, schema_p s, record r, int offset) {
    int rec_val;
    page_set_pos_begin(curr);
    int first_pos = page_current_pos(curr);
    while (1) {
        rec_val = page_get_int(curr);
        if (val == rec_val) {
            page_set_current_pos(curr, first_pos + offset);
            get_page_record(curr, r, s);
            return 1;
        }
        curr = get_page_for_next_record(s);
        first_pos = page_current_pos(curr);
    }
}
...
tbl_p table_search (tbl_p t, char *attr, char *op, int val)
{
  ...
  set_tbl_position ( t, TBL_BEG );
  if (strcmp(op, ">") == 0) {
      while ( find_record_int_val (rec, s, f->offset, int_y_larger, val) )
        {
      ...
      while ( find_record_int_val (rec, s, f->offset, int_x_larger, val) )
        {
          put_record_info (DEBUG, rec, s);
          append_record ( rec, res_sch );
        }
      ...
      while ( find_record_int_val (rec, s, f->offset, int_x_larger_or_equal, val) )
        {
          put_record_info (DEBUG, rec, s);
          append_record ( rec, res_sch );
        }
      ...
      while ( find_record_int_val (rec, s, f->offset, int_y_larger_or_equal, val) )
        {
          put_record_info (DEBUG, rec, s);
          append_record ( rec, res_sch );
        }
      ...
      while ( find_record_int_val (rec, s, f->offset, int_not_equal, val) )
        {
          put_record_info (DEBUG, rec, s);
          append_record ( rec, res_sch );
        }
      ...
      // UNCOMMENT LINE BELOW TO TEST FOR LINEAR SEARCH
      /*while ( find_record_int_val (rec, s, f->offset, int_equal, val) )*/
      if (binary_find_record_int_val (rec, s, f->offset, int_equal, val) == 1)
      {
          put_record_info (DEBUG, rec, s);
          append_record ( rec, res_sch );
      }
...
schema_p join_schemas (tbl_p left, tbl_p right, char *common_name, char* sch_name) {
    put_schema_info(FATAL, left->sch);
    printf("\n\n");
    printf("\n\n");
    printf("\n\n");

    schema_p join_schema = new_schema(sch_name);
    field_desc_p desc = left->sch->first;
    for (; desc != NULL; desc = desc->next) {
        add_field(join_schema, copy_field(desc));
    }
    desc = right->sch->first;
    for (; desc != NULL; desc = desc->next) {
        if (strcmp(desc->name, common_name) != 0) {
            add_field(join_schema, copy_field(desc));
        }
    }
    ...
    printf("\n\n");
    printf("\n\n");
    put_schema_info(FATAL, right->sch);
    printf("\n\n");
    printf("\n\n");
    put_schema_info(FATAL, join_schema);
    return join_schema;
}

void concat_rec (tbl_p left, tbl_p right, record r_left, record r_right, record res, char *common_name) {
    int i = 0, j = 0;
    field_desc_p f = left->sch->first;
    for (; f != NULL; f = f->next) {
        bcopy(r_left[i], res[j], f->len);
        i++;
        j++;
    }
    i = 0;
    f = right->sch->first;
    for (; f != NULL; f = f->next) {
        if (strcmp(f->name, common_name) == 0) {
            printf("Common name: %s\n", f->name);
            i++;
            continue;
        }
        bcopy(r_right[i], res[j], f->len);
        i++;
        j++;
    }
...
char * common_name (tbl_p left, tbl_p right, int *offset_left, int *offset_right) {

    field_desc_p desc_left = left->sch->first;

    while (desc_left != NULL) {

        field_desc_p desc_right = right->sch->first;

        while (desc_right != NULL) {

            if (strcmp(desc_left->name, desc_right->name) == 0 && desc_left->type == INT_TYPE) {

                *offset_left = desc_left->offset;
                *offset_right = desc_right->offset;
                char * common_name = malloc(sizeof(char)*strlen(desc_left->name));
                strcpy(common_name, desc_left->name);
                return common_name;
            }
            desc_right = desc_right->next;
        }
        desc_left = desc_left->next;
    }
    return "";
}

int nested_join(tbl_p left, tbl_p right, schema_p result_sch, int offset_left, int offset_right, char *common_name) {

    int tmp, rec_val, pos_left, pos_right, tmp_right, rec_val_right;

    record rec_left = new_record(left->sch);
    record rec_right = new_record(right->sch);
    record res = new_record(result_sch);


    set_tbl_position(left, TBL_BEG);
    pos_left = page_set_current_pos(left->current_pg, SEEK_SET + PAGE_HEADER_SIZE);

    while (get_record (rec_left, left->sch)) {
        // While left table has records

        page_p pg_left = get_page_for_next_record(left->sch);
        if (pg_left == NULL)
            break;
        left->current_pg = pg_left;


        tmp = page_current_pos(left->current_pg);
        pos_left = tmp;

        rec_val = page_get_int_at(left->current_pg, pos_left + offset_left);

        page_set_current_pos(left->current_pg, tmp);

        set_tbl_position(right, TBL_BEG);
        pos_right = page_set_current_pos(right->current_pg, SEEK_SET + PAGE_HEADER_SIZE);

        while ( get_record ( rec_right, right->sch)) {

            page_p pg_right = get_page_for_next_record(right->sch);
            right->current_pg = pg_right;
            if (pg_right == NULL) {
                break;
            }

            tmp_right = page_current_pos(right->current_pg);
            pos_right = tmp_right;

            rec_val_right = page_get_int_at(right->current_pg, pos_right + offset_right);

            if (rec_val == rec_val_right) {
                concat_rec (left, right, rec_left, rec_right, res, common_name);
                append_record(res, result_sch);
            }
            page_set_current_pos(right->current_pg, tmp_right);
...
schema_p block_nested_join(tbl_p left, tbl_p right, schema_p res_sch, int offset_left, int offset_right, char *common_name) {
    //TODO: Implement block nested join
    int l_first_blk, l_last_blk, r_first_blk, r_last_blk
        l_rec_val, r_rec_val, l_pos, r_pos;
    page_p l_first, l_last, r_first, r_last;
    record left_rec, right_rec;


    l_first = get_page_for_next_record(left->sch);
    l_last = get_page(left->sch->name, -1);
    l_first_blk = page_block_nr(l_first);
    l_last_blk = page_block_nr(l_last);

    r_first = get_page_for_next_record(right->sch);
    r_last = get_page(left->sch->name, -1);
    r_first_blk = page_block_nr(r_first);
    r_last_blk = page_block_nr(r_last);

    left_rec = new_record(left->sch);
    right_rec = new_record(right->sch);

    for (; l_first_blk != l_last_blk; l_first_blk++) {
      for (; r_first_blk != r_last_blk; r_first_blk++) {
        while (get_record(left_rec, left->sch)) {
          l_pg = get_page_for_next_record(left->sch);
          if (l_pg == NULL)
            break;

            left->current_pg = l_pg;

            l_pos = page_current_pos(left->current_pg);
            l_rec_val = page_get_int_at(left->current_pg, l_pos + offset_left)
          while (get_record(right_rec, right->sch)) {
...
tbl_p table_natural_join (tbl_p left, tbl_p right)
{
    ...
    char* new_sch_name = "joined_sch";
    schema_p new_sch;

    char *name = common_name(left, right, &offset_left, &offset_right);
    printf("NAME = %s\n", name);
    new_sch = join_schemas (left, right, name, new_sch_name);


    nested_join(left, right, new_sch, offset_left, offset_right, name);
