...
typedef struct page_struct {
  ...
  int pinned;      /**< non-zero if the block is pinned to the page */
...
static void put_fhandle_info ( pmsg_level level, fhandle_p fh )
{
  ...
  append_msg (level, "%d blocks, current: %d.\n",
	     fh->num_blocks, fh->current_block->blk_nr);
...
int get_free_pos(page_p p) {
    int r = p->free_pos + PAGE_HEADER_SIZE;
    return r;
}

int get_page_num(page_p p) {
    return p->page_nr;
}

void put_page_pos(pmsg_level level, page_p p) {
    put_msg(level, "Current pos: %d\n", p->current_pos);
}

void print_int(pmsg_level level, int val) {
    put_msg(level, "Value: %d\n", val);
}
...
static void init_page (page_p p, int page_nr)
{
  if ( p == NULL )
    return;
  p->page_nr = page_nr;
...
{
  int i, num_compared;
  for ( i = 0, num_compared = 0; num_compared < num_file_handles; i++ )
    {
      if ( file_handles[i] != NULL )
	{
...
page_p get_page ( const char *fname, int blknr )
{
  ...
  else if ( fh->current_block != NULL && blknr == fh->current_block->blk_nr )
